      ...
      try {
        ...
        const [extractCategories, extractThreads] = await Promise.all([
          ...
          apiGet<ThreadSummary[]>(apiClient, "/api/threads/threads", {
            params: {
              category: activeCategory && activeCategory !== null ? activeCategory : undefined,
              q: search || undefined,
            },
          }),
        ]);
        ...
        console.log(error);
        toast.error("error", {
          description: `${error} apiGet Failed`,
        });
      } finally {
        setIsLoading(false);
      }
          ...
          {!isLoading && threads.length === 0 && (
            <Card className="border-dashed border-border bg-card">
              <CardContent className="py-10 text-center">
                <p className="text-sm text-muted-foreground">No threads found. Create your first thread</p>
              </CardContent>
            </Card>
          )}
          {!isLoading &&
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                          ...
                          {thread?.author?.handle && (
                            <span className="text-muted-foreground/90">by @{thread?.author?.handle}</span>
                          )}
