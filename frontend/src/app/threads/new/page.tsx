  ...
  async function onThreadSubmit(values: NewThreadFormValues) {
    try {
      ...
      toast.success("New thread created successfully", {
        description: "Your thread is live now!",
      });
      router.push("/");
      ...
      <Card className="border-border/70 bg-card">
        ...
        <CardContent>
          <form onSubmit={form.handleSubmit(onThreadSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              <label htmlFor="categorySlug" className="text-sm font-semibold text-foreground">
                Category
              </label>
              <select
                id="categorySlug"
              ...
              >
                {categories.map((category) => (
                  <option value={category.slug} id={category.slug} key={category.slug}>
                    {category.name}
                  </option>
                ))}
              </select>
