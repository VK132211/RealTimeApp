...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    function handlePresense(payload: { onlineUserIds?: string[] }) {
      const list = payload?.onlineUserIds ?? [];
      // setOnlineUserIds(list);
      setOnlineUserIds(list.map(String));
  ...
  const onlineCount = users.filter((u) => onlineUserIds.includes(String(u.id))).length;
