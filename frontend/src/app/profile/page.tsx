...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      try {
        setIsLoading(true);
        const getUserInfo = await apiGet<UserProfileResponse>(apiClient, "/api/me");
  ...
  }, [isLoaded, isSignedIn, apiClient, form]);
            ...
            <CardHeader className="pb-4">
              <div className="flex items-start gap-6">
                <Avatar className="h-20 w-20">
                  {avatarValue && <AvatarImage src={avatarValue || "/placeholder.xyz"} alt={displayNameValue ?? ""} />}
                </Avatar>
