...
export default function NotificationsPage() {
  function formatText(n: Notification) {
    const actor =
      n.actor.handle !== null && n.actor.handle !== "" ? `@${n.actor.handle}` : n.actor.displayName ?? "Someone";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const data = await apiGet<Notification[]>(apiClient, "/api/notifications");
    ...
    try {
      if (!n.readAt) {
        ...
        setNotifications((prev) =>
          prev.map((noti) => (noti.id === n.id ? { ...n, readAt: new Date().toISOString() } : n))
        );
        decrementUnread();
      }
    } catch (err) {
